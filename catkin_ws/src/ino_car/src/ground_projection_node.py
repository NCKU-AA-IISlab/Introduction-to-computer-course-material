...
import numpy as np
from ground_projection.GroundProjection1 import GroundProjection
...
class GroundProjectionNode(object):
    ...
    def lineseglist_cb(self,seglist_msg):
        ...
        if self.hasleft and self.hasright: 
            goal_out.x = sum(c.points[i].x  for i,c  in enumerate(new_seglist.lanelines))/4
            goal_out.y = sum(c.points[i].y  for i,c  in enumerate(new_seglist.lanelines))/4

        if self.hasleft and not self.hasright: 
            goal_out.x = sum(c.points[i].x  for i,c  in enumerate(new_seglist.lanelines))/2
            goal_out.y = sum(c.points[i].y  for i,c  in enumerate(new_seglist.lanelines))/2
            goal_out.y = goal_out.y - self.lane_width
       
        if not self.hasleft and  self.hasright: 
            goal_out.x = sum(c.points[i].x  for i,c  in enumerate(new_seglist.lanelines))/2
            goal_out.y = sum(c.points[i].y  for i,c  in enumerate(new_seglist.lanelines))/2
            goal_out.y = goal_out.y + self.lane_width
        goal_out.x = goal_out.x + self.lookahead_dis
        markers = rviz_tools.RvizMarkers(self.robot_name + '/camera_link', 'visualization_marker')   
        # Publish a sphere using a ROS Point
        point = goal_out
        markers.publishSphere(point,  'orange', Vector3(0.1,0.1,0.1), 5.0) # pose, color, scale, lifetime
